...
const Hero = () => {
  const discountD = useAnimation();
  const headingC1 = useAnimation();
  const headingC2 = useAnimation();
  const headingC3 = useAnimation();
  const paragraphC = useAnimation();
  const buttonC = useAnimation();

  const animationSequence = async () => {
    await headingC1.start({ opacity: 1, scale: 1 });
    await headingC2.start({ opacity: 1, scale: [1.5, 1] });
    await headingC3.start({ opacity: 1, scale: 1 });
    await paragraphC.start({ opacity: 1, scale: 1 });
    await discountD.start({ opacity: 1, scale: 1 });
    return await buttonC.start({ opacity: 1, scale: [1.5, 1, 1.3, 1] });
  };
